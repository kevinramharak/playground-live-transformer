import ts, { getIndentString } from 'typescript';
 ...
 * 
 */
function createTransformerFactory(_ts: typeof ts, output: { blocks: ts.Block[], imports: ts.ImportDeclaration[] }) {
    return function transformerFactory(context: ts.TransformationContext) {
        return function transformer(sourceFile: ts.SourceFile): ts.SourceFile {
            ...
            function visitor(node: ts.Node): ts.Node | undefined {
                ...
                if (_ts.isBlock(node)) {
                    const comments = _ts.getLeadingCommentRanges(sourceFileText, node.getFullStart()) || [];
                    const multiline = comments.filter(comment => comment.kind === _ts.SyntaxKind.MultiLineCommentTrivia);
                    if (multiline.some(range => {
                        const content = sourceFileText.slice(range.pos, range.end);
                        // https://regexr.com/5ohbm
                ...
                return _ts.visitEachChild(node, visitor, context);
            }
            
            return _ts.visitNode(sourceFile, visitor);
        }
    }
}

function evaluateTransformer(_ts: typeof ts, imports: ts.ImportDeclaration[], code: string) {
    ...
    try {
        ...
        const result = closure(_ts, module, module.exports);
    ...
    return {
        ...
        willMount(sandbox, container) {
            ...
            const $button = ds.button({
                ...
                async onclick() {
                    const output: Parameters<typeof createTransformerFactory>[1] = { blocks: [], imports: [] };
                    const transformerFactory = createTransformerFactory(ts, output);
                    ...
                    const evaluated = evaluateTransformer(ts, output.imports, compiledOutput) as { default: (program: ts.Program) => ts.TransformerFactory<ts.SourceFile> };
